...
export function BallCard({ ball, onDelete, onEdit }: BallCardProps) {
  ...
  return (
    <Card className="overflow-hidden hover:shadow-xl transition-shadow">
      <div className="h-48 bg-gradient-to-br from-bowling-blue to-blue-600 relative">
        <div className="absolute inset-0 bg-black bg-opacity-20" />
        <div className="absolute bottom-4 left-4 text-white">
          <h3 className="text-xl font-bold">{ball.name}</h3>
          <p className="text-blue-100">{ball.brand}</p>
        </div>
      </div>
      ...
      <CardContent className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            <span className="text-sm text-gray-600 capitalize">{ball.coreType} {ball.coverstockType}</span>
          </div>
          <div className="flex space-x-2">
            {onEdit && (
              <Button variant="ghost" size="sm" onClick={onEdit}>
                <Edit className="h-4 w-4 text-bowling-blue" />
              </Button>
            )}
            <Button variant="ghost" size="sm" onClick={onDelete}>
              <Trash2 className="h-4 w-4 text-pin-red" />
            </Button>
          </div>
        </div>
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">{ball.surface}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Drilling:</span>
            <span className="font-medium">{ball.drilling}</span>
          </div>
          <div className="flex justify-between">
